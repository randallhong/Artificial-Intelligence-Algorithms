...
if __name__ == "__main__":
    ...
    while 1:
        ...
        while 1:
            ...
            fprime =t*c#problem!!! liexiangliang zhuanzhi wenti hang chengyi lie
            ...
            for j in range(0,numplanes):
                fprime_for_plane_j = a[:,j]/(a[:,j].T*x-b[j])#fuhao wenti

                fprimeprime_for_plane_j = (a[:,j]*a[:,j].T)/((a[:,j].T*x-b[j])*(a[:,j].T*x-b[j]))
        ...
        duality_gap =numplanes/t#(1-t)*c.T*x+numplanes*np.log(-a[:,num_outer_iterations].T*x + b[num_outer_iterations])

...
class BayesianNetwork:
    ...
    def __init__(self):
        ...
        v4=Variable("Wetglass",parents2,pro4)
...
def Weight_sample(b,evidence):
    ...
    for i in range(0,len(v)):
        for j in range(0,len(evidence)):
            if evidence[j][0][0]==i:
                v_evidence=True
                if len(v[evidence[j][0][0]].parents)==0:
                    for x in range(0,len(v[evidence[j][0][0]].cpt)):
                        if v[evidence[j][0][0]].cpt[x][1][0]==evidence[j][1][0]:
                            if evidence[j][1][0]=='t':
                                v[evidence[j][0][0]].happen.append('t')
                                
                                w*=v[evidence[j][0][0]].cpt[x][2][0]
                                break
                            else:
                                v[evidence[j][0][0]].happen.append('f')
                                w*=1-v[evidence[j][0][0]].cpt[x][2][0]
                elif len(v[evidence[j][0][0]].parents)==1:
                    for x in range(0,len(v[evidence[j][0][0]].cpt)):
                        if v[evidence[j][0][0]].parents[0].happen[0]==v[evidence[j][0][0]].cpt[x][1][0]:
                            if evidence[j][1][0]=='t':
                                v[evidence[j][0][0]].happen.append('t')
                                w*=v[evidence[j][0][0]].cpt[x][2][0]
                            else:
                    ...
                    for x in range(0,len(v[evidence[j][0][0]].cpt)):
                        if v[evidence[j][0][0]].parents[0].happen[0]==v[evidence[j][0][0]].cpt[x][1][0] and v[evidence[j][0][0]].parents[1].happen[0]==v[evidence[j][0][0]].cpt[x][1][1]:
                            if evidence[j][1][0]=='t':
                                v[evidence[j][0][0]].happen.append('t')
                                w*=v[evidence[j][0][0]].cpt[x][2][0]
                            else:
...
def Likelihood_weighting(x,e,bn,N):
    ...
    for i in range(0,int(N)):
        event=Weight_sample(bn,e)
        if event[x[0]].happen[0]=='t':
            T+=event[4]
        else:
            F+=event[4]
    print(T)
    print(F)
    Normalize[0][0].append(event[x[0]].name)
    Normalize[1][0].append(event[x[0]].name)
    Normalize[0][1].append("Flase")
    Normalize[1][1].append("True")
    Normalize[0][2].append(F/(T+F))
    Normalize[1][2].append(T/(T+F))
    return Normalize
...
def ReadFile(filepath):
    ...
    #inference[[[3]],[[[0],["t"]],[[2],["f"]]],[10]]
    print(inference)
    return inference
if  __name__=="__main__":
    bn=BayesianNetwork()
    filepath='/Users/apple/Desktop/inference2.txt'
    inf=ReadFile(filepath)
    # e=Weight_sample(bn,inf[1])
    sum=0
    for times in range(0,10):
        NOR=Likelihood_weighting(inf[0],inf[1],bn,inf[2][0])
        sum+=NOR[1][2][0]
    print(sum/10)